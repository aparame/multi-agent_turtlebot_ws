...
Detects TurtleBots as dark blobs on the checkerboard floor using:
  ...
  3. History-aware identity tracking with short-horizon prediction
...
class RobotDetector:
    ...
    def __init__(self, config_path: str, calibration_path: str):
        ...
        self.max_match_dist_px = float(det_cfg.get("max_match_distance_px", 150))
        self.position_history_size = max(
            2, int(track_cfg.get("position_history_size", 8)))
        self.prediction_horizon_sec = max(
            0.0, float(track_cfg.get("prediction_horizon_sec", 1.0)))
        self.prediction_weight = min(
            1.0, max(0.0, float(track_cfg.get("prediction_weight", 0.8))))
        self.missed_track_cost_px = float(
            track_cfg.get("missed_track_cost_px", self.max_match_dist_px * 1.25))
    ...
    # ------------------------------------------------------------------
    # Identity tracking (history-aware prediction)
    # ------------------------------------------------------------------
    def _remember_observation(self, tr: TrackedRobot, timestamp: float,
                              px: int, py: int, wx: float, wy: float,
                              reset: bool = False):
        if reset:
            tr.position_history.clear()
        tr.position_history.append(
            (timestamp, float(px), float(py), float(wx), float(wy)))
        if len(tr.position_history) > self.position_history_size:
            del tr.position_history[:-self.position_history_size]

    def initialize_track(self, robot_id: int, wx: float, wy: float,
                         px: int, py: int, timestamp: Optional[float] = None):
        """Initialize one robot identity from a known blob assignment."""
        if robot_id not in self.tracked:
            return
        now = time.time() if timestamp is None else timestamp
        tr = self.tracked[robot_id]
        tr.smoothed_x = wx
        tr.smoothed_y = wy
        tr.pixel_x = px
        tr.pixel_y = py
        ...
        tr.initialized = True
        tr.lost_count = 0
        self._remember_observation(tr, now, px, py, wx, wy, reset=True)

    def _predicted_pixel(self, tr: TrackedRobot,
                         timestamp: float) -> Tuple[float, float]:
        """
        Predict the next blob location using recent centroid history.

        This short constant-velocity prediction keeps identities stable when
        two robots pass close together and the nearest current centroid is
        briefly ambiguous.
        """
        history = tr.position_history
        if len(history) < 2:
            return float(tr.pixel_x), float(tr.pixel_y)

        latest_t, latest_px, latest_py, _latest_wx, _latest_wy = history[-1]
        first = None
        for sample in history:
            if latest_t - sample[0] > 1e-3:
                first = sample
                break
        if first is None:
            return latest_px, latest_py

        dt_history = latest_t - first[0]
        vx = (latest_px - first[1]) / dt_history
        vy = (latest_py - first[2]) / dt_history
        dt_predict = min(
            max(0.0, timestamp - latest_t), self.prediction_horizon_sec)
        return latest_px + vx * dt_predict, latest_py + vy * dt_predict

    def _blob_track_cost(self, tr: TrackedRobot,
                         blob: Tuple[int, int, float],
                         timestamp: float) -> float:
        cx, cy, _area = blob
        pred_x, pred_y = self._predicted_pixel(tr, timestamp)
        predicted_dist = math.hypot(cx - pred_x, cy - pred_y)
        last_dist = math.hypot(cx - tr.pixel_x, cy - tr.pixel_y)
        if len(tr.position_history) < 2:
            return last_dist
        return (self.prediction_weight * predicted_dist +
                (1.0 - self.prediction_weight) * last_dist)
    ...
    def initialize_identities(self, blobs: List[Tuple[int, int, float]],
        ...
        for rid, (target_x, target_y) in initial_positions.items():
            ...
            if best_idx >= 0:
                ...
                self.initialize_track(rid, wx, wy, cx, cy)

    def _match_blobs_to_tracks(self, blobs: List[Tuple[int, int, float]],
                               timestamp: float):
        """
        Match blobs to existing tracks with global assignment.

        Costs are based mostly on the predicted pixel position from each
        robot's recent history. This avoids identity swaps when robots pass
        close enough that greedy nearest-neighbour matching becomes ambiguous.
        """
        ...
        track_items = sorted(active_tracks.items(),
                             key=lambda kv: kv[1].last_seen, reverse=True)
        best_cost = float("inf")
        best_assignment: Dict[int, Optional[int]] = {}
        best_assigned_count = -1

        def search(track_idx: int, used_blobs: set,
                   current: Dict[int, Optional[int]],
                   total_cost: float, assigned_count: int):
            nonlocal best_cost, best_assignment, best_assigned_count
            if track_idx >= len(track_items):
                if (total_cost < best_cost or
                        (math.isclose(total_cost, best_cost) and
            ...
            rid, tr = track_items[track_idx]
            candidates = []
            for blob_idx, blob in enumerate(blobs):
                if blob_idx in used_blobs:
                    continue
                cost = self._blob_track_cost(tr, blob, timestamp)
                if cost <= self.max_match_dist_px:
                    candidates.append((cost, blob_idx))
            candidates.sort()

            for cost, blob_idx in candidates:
                current[rid] = blob_idx
                used_blobs.add(blob_idx)
                search(track_idx + 1, used_blobs, current,
                       total_cost + cost, assigned_count + 1)
                used_blobs.remove(blob_idx)
                current.pop(rid, None)

            current[rid] = None
            search(track_idx + 1, used_blobs, current,
                   total_cost + self.missed_track_cost_px, assigned_count)
            current.pop(rid, None)
        ...
        return {
            rid: blobs[blob_idx]
            for rid, blob_idx in best_assignment.items()
    ...
    def detect(self, frame: np.ndarray) -> Dict[int, RobotPose]:
        ...
        assignments = self._match_blobs_to_tracks(blobs, now)
        ...
        for rid, (cx, cy, area) in assignments.items():
            ...
            tr.last_seen = now
            tr.lost_count = 0
            self._remember_observation(tr, now, cx, cy, wx, wy)
    ...
    def draw_detections(self, frame: np.ndarray,
        ...
        for rid, pose in sorted(poses.items()):
            color = ROBOT_COLORS.get(rid, (255, 255, 255))
